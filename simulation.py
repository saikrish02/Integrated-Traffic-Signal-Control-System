...
speeds = {'car':2.25, 'bus': 1.8, 'truck':1.8, 'auto':2, 'pickup' : 1.9, 'rickshaw':2, 'bike':2.5}  # average speeds of vehicles
...
vehicleTypes = {0:'car', 1:'bus', 2:'truck', 3:'rickshaw', 4:'bike', 5:'auto', 6:'pickup'}
directionNumbers = {0:'right', 1:'down', 2:'left', 3:'up'}

# Coordinates of signal image, timer, and vehicle count
signalCoods = [(530,230),(810,230),(810,570),(530,570)]
signalTimerCoods = [(530,210),(810,210),(810,550),(530,550)]
greenTimeCoods = [(300,250),(1000,250),(1000,600),(300,600)]
    ...
    def move(self):
        ...
        if(self.direction=='right'):
            if(self.crossed==0 and self.x+self.currentImage.get_rect().width>stopLines[self.direction]):   # if the image has crossed stop line now
                ...
                print("green", signals[currentGreen].green)
...
def setTime():
    global noOfCars, noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfAutos, noOfPickups, noOfLanes
    global carTime, busTime, truckTime, rickshawTime, bikeTime, pickupTime, autoTime
    greenTime = [0, 0, 0, 0]
    weightage = [0, 0, 0, 0]
    ...
    while(True):
        noOfCars, noOfBuses, noOfTrucks, noOfRickshaws, noOfBikes, noOfPickups, noOfAutos = 0,0,0,0,0,0,0
        for j in range(len(vehicles[directionNumbers[temp]][0])):
            vehicle = vehicles[directionNumbers[temp]][0][j]
            if vehicle.crossed==0:
                vclass = vehicle.vehicleClass
                # print(vclass)
                noOfBikes += 1
        for i in range(1,3):
            for j in range(len(vehicles[directionNumbers[temp]][i])):
                vehicle = vehicles[directionNumbers[temp]][i][j]
                if(vehicle.crossed==0):
                    vclass = vehicle.vehicleClass
                    # print(vclass)
                    if(vclass=='car'):
                        ...
                        noOfPickups += 1
        # print(noOfCars)
        print(noOfBikes, noOfAutos, noOfRickshaws, noOfCars, noOfPickups, noOfBuses, noOfTrucks)
        weightage[temp] = ((noOfCars*carTime) + (noOfRickshaws*rickshawTime) + (noOfBuses*busTime) + (noOfTrucks*truckTime)+ (noOfBikes*bikeTime) + (noOfAutos*autoTime) + (noOfPickups*pickupTime))
        if temp == 0:
            break
        temp += 1
        temp %= noOfSignals
    ...
    for i in range(0, 4):
        sum += weightage[i]
    for i in range(0, 4):
        weightage[i] = weightage[i] / sum
    for i in range(0, 4):
        weightage[i] = round(weightage[i] * 80)
        if(weightage[i]<defaultMinimum):
            weightage[i] = defaultMinimum
        elif(weightage[i]>defaultMaximum):
            weightage[i] = defaultMaximum
    for i in range(0, 4):
        signals[i].green = weightage[i]
        signalTimer[i] = weightage[i]
    temp = 1
    red = 0
    signals[2].red = signals[1].green + signals[1].yellow
    signals[3].red = signals[2].green + signals[2].yellow + signals[2].red
    signals[0].red = signals[3].green + signals[3].yellow + signals[3].red
    signals[1].red = signals[0].green + signals[0].yellow + signals[0].red


def repeat():
    global currentGreen, currentYellow, violationCount, nextGreen, noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfAutos, noOfCars, noOfPickups
    print(noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfAutos, noOfCars, noOfPickups)
    while(signals[currentGreen].green>0):   # while the timer of current green signal is not zero
        printStatus()
        updateValues()
        time.sleep(1)
    currentYellow = 1   # set yellow signal on
    print("yes " , currentGreen)
    ...
    while signals[currentGreen].yellow>0:  # while the timer of current yellow signal is not zero
        ...
        time.sleep(1)
    if currentGreen == 0:
        thread = threading.Thread(name="SetTime", target=setTime, args=())
        thread.daemon=True
        thread.start()
...
def simulationTime():
    global timeElapsed, simTime, violationCount
    while(True):
        timeElapsed += 1
        time.sleep(1)
...
class Main:
    ...
    while True:
        ...
        for i in range(0,noOfSignals):  
            ...
            signalTimerTexts[i] = font.render("Green Time : " + str(signalTimer[i]), True, (0, 255, 0), black)
            screen.blit(signalTimerTexts[i], greenTimeCoods[i])
        ...
        violationText = font.render(("Violation Count: " + str(violationCount)), True, (255, 0, 0), white)
        screen.blit(violationText, (1100, 100))
